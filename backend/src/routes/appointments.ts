...
appointmentsRouter.get('/', async (c) => {
  ...
  try {
    const rows = date
      ? await sql`SELECT ${sql.unsafe(fields)} FROM appointments a JOIN patients p ON p.id = a.patient_id WHERE a.provider_id = ${providerId} AND a.facility_id = ${c.get('facilityId') as string} AND a.scheduled_at >= ${date}::date AND a.scheduled_at < (${date}::date + INTERVAL '1 day') ORDER BY a.scheduled_at`
      : await sql`SELECT ${sql.unsafe(fields)} FROM appointments a JOIN patients p ON p.id = a.patient_id WHERE a.provider_id = ${providerId} AND a.facility_id = ${c.get('facilityId') as string} ORDER BY a.scheduled_at`;
...
appointmentsRouter.post('/', async (c) => {
  ...
  try {
    const patient = await sql`SELECT id FROM patients WHERE id = ${patientId} AND facility_id = ${c.get('facilityId') as string}`;
    if (!patient.length) return c.json({ error: 'Patient not found' }, 404);
    const rows = await sql`INSERT INTO appointments (patient_id, provider_id, facility_id, scheduled_at, duration_minutes, reason) VALUES (${patientId}, ${providerId}, ${c.get('facilityId') as string}, ${body.scheduled_at}, ${duration}, ${reason}) RETURNING *`;
...
appointmentsRouter.put('/:id', async (c) => {
  ...
  try {
    const rows = await sql`UPDATE appointments SET status = COALESCE(${status ?? null}, status), notes = COALESCE(${notes ?? null}, notes), updated_at = NOW() WHERE id = ${c.req.param('id')} AND provider_id = ${providerId} AND facility_id = ${c.get('facilityId') as string} RETURNING *`;
...
appointmentsPortalRouter.post('/me/appointments', async (c) => {
  ...
  try {
    ...
    const rows = await sql`INSERT INTO appointments (patient_id, provider_id, facility_id, scheduled_at, reason) SELECT ${patient.id}, ${providerId}, facility_id, ${body.scheduled_at}, ${reason} FROM patients WHERE id = ${patient.id} RETURNING *`;
